// Quantai - Gesture Service
// Mobile gesture handling for AI/ML platform
...
export class GestureService {
  ...
  private registerDefaultGestures(): void {
    this.registerGesture({ id: 'swipe_left_hide', type: 'swipe', priority: 10, enabled: true, options: { direction: 'left', action: 'hide' } });
    this.registerGesture({ id: 'swipe_right_bookmark', type: 'swipe', priority: 10, enabled: true, options: { direction: 'right', action: 'bookmark' } });
    this.registerGesture({ id: 'double_tap_run_again', type: 'double_tap', priority: 5, enabled: true, options: { action: 'run_again' } });
    this.registerGesture({ id: 'long_press_view_details', type: 'long_press', priority: 8, enabled: true, options: { action: 'view_details' } });
