// Quantai - Share Extension Service
// Mobile share extension for AI/ML platform
...
export class ShareExtensionService {
  ...
  private registerDefaultTargets(): void {
    const targets: ShareTarget[] = [
      { id: 'import_data', contentTypes: ['text', 'url', 'image', 'file'], maxFileSize: 52428800, maxFiles: 10, description: 'Import Dataset' },
      { id: 'quick_save', contentTypes: ['text', 'url', 'image', 'video', 'file'], maxFileSize: 104857600, maxFiles: 20, description: 'Save to quantai' },
    ];
  ...
  public async prepareCompose(content: SharedContent): Promise<ComposeData> {
    ...
    switch (content.type) {
      ...
      case 'file':
        ...
        compose.title = content.fileName || 'Import Dataset';
        break;
    }
    return compose;
  }
