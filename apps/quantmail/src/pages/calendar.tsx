...
export const CalendarPage: React.FC<CalendarPageProps> = ({ userId }) => {
  ...
  const handleCreateEvent = useCallback(async () => {
    ...
    setCreating(true);
    try {
      const startTime = createForm.isAllDay
        ? `${createForm.startDate}T00:00:00`
        : `${createForm.startDate}T${createForm.startTime}:00`;
      const endTime = createForm.isAllDay
        ? `${createForm.endDate || createForm.startDate}T23:59:59`
        : `${createForm.endDate || createForm.startDate}T${createForm.endTime}:00`;
    ...
    <div className="calendar-page">
      <header className="calendar-header">
        <div className="calendar-nav">
          <button onClick={navigatePrev} className="nav-btn">
            &lt;
          </button>
          <h2>
            {viewMode === 'day'
                      ...
                      <div className="cell-events">
                        {dayEvents.slice(0, 3).map((event) => (
                          <div
                            ...
                            style={{
                              backgroundColor: CATEGORY_COLORS[event.category] || '#4285f4',
                            }}
                            draggable
              ...
              <div className="week-body">
                {hours.map((hour) => (
                  <div key={hour} className="week-row">
                    ...
                    {weekDays.map((date) => {
                      const cellEvents = getEventsForDate(date).filter(
                    ...
                    >
                      {hourEvents.map((event) => (
                        ...
                        >
                          ...
                          {event.videoCallUrl && (
                            <a href={event.videoCallUrl} className="join-call-btn">
                              Join Call
                            </a>
                          )}
                        </div>
      ...
      {selectedEvent && (
        <div className="event-detail-modal" onClick={() => setSelectedEvent(null)}>
          <div className="event-detail" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="event-time-range">
              {new Date(selectedEvent.startTime).toLocaleString()} -{' '}
              {new Date(selectedEvent.endTime).toLocaleString()}
            </p>
            {selectedEvent.location && (
              <p className="event-location-detail">Location: {selectedEvent.location}</p>
            )}
            ...
            {selectedEvent.videoCallUrl && (
              <a href={selectedEvent.videoCallUrl} className="join-call-link">
                Join Video Call
              </a>
            )}
            <div className="event-actions">
              <button onClick={() => handleDeleteEvent(selectedEvent.id)} className="delete-btn">
                Delete
              </button>
          ...
          <div className="create-event-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <label className="checkbox-label">
              <input
                ...
                onChange={(e) => setCreateForm((p) => ({ ...p, isAllDay: e.target.checked }))}
              />{' '}
              All day
            </label>
            <div className="form-group">
            ...
            <div className="form-group">
              ...
              <input
                ...
                onChange={(e) => setCreateForm((p) => ({ ...p, attendees: e.target.value }))}
            ...
            <div className="form-row">
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setCreateForm((p) => ({ ...p, recurrencePattern: e.target.value }))
                  }
                >
            ...
            <div className="form-row">
              <label className="checkbox-label">
                <input
                  type="checkbox"
                  checked={createForm.addVideoCall}
                  onChange={(e) => setCreateForm((p) => ({ ...p, addVideoCall: e.target.checked }))}
                />{' '}
                Add video call
              </label>
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setCreateForm((p) => ({ ...p, reminderMinutes: Number(e.target.value) }))
                  }
                >
            ...
            <div className="modal-actions">
              ...
              <button onClick={handleCreateEvent} disabled={creating || !createForm.title.trim()}>
