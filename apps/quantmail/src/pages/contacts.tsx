    ...
    <div className="contacts-page">
      <header className="contacts-header">
        ...
        <div className="header-actions">
          <button onClick={() => setShowAddForm(true)} className="add-contact-btn">
            + Add Contact
          </button>
          <label className="import-btn">
            ...
            <input
              ...
              hidden
              onChange={(e) => {
                if (e.target.files?.[0]) handleImportContacts(e.target.files[0]);
              }}
            />
          </label>
      ...
      <div className="contacts-layout">
        <aside className="groups-sidebar">
          ...
          <button
            ...
            className={`group-item ${selectedGroup === 'favorites' ? 'active' : ''}`}
          >
            Favorites{' '}
            <span className="group-count">{contacts.filter((c) => c.isFavorite).length}</span>
          </button>
          {groups.map((g) => (
            <button
              key={g.id}
        ...
        <main className="contacts-main">
          <div className="contacts-toolbar">
            ...
            <select
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value as 'name' | 'email' | 'recent')}
            >
          ...
          {loading ? (
            <div className="loading-state">
              {Array.from({ length: 8 }).map((_, i) => (
                <div key={i} className="contact-skeleton"></div>
              ))}
            </div>
          ) : filteredContacts.length === 0 ? (
            <div className="empty-state">
              <h3>No contacts found</h3>
              <p>Add your first contact or adjust your search.</p>
            </div>
          ) : (
                    ...
                    >
                      ...
                      <div className="contact-badges">
                        {isBirthdayToday(contact.birthday) && (
                          <span className="birthday-badge" title="Birthday today!">
                            &#x1F382;
                          </span>
                        )}
            ...
            <div className="detail-fields">
              ...
              {selectedContact.birthday && (
                <div className="field">
                  ...
                  <span>{new Date(selectedContact.birthday).toLocaleDateString()}</span>
                </div>
              )}
          ...
          <div className="add-contact-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="form-group">
              ...
              <input
                ...
                onChange={(e) => setAddForm((p) => ({ ...p, birthday: e.target.value }))}
              />
            </div>
            <div className="form-group">
              <label>Notes</label>
            ...
            <div className="modal-actions">
              ...
              <button
                ...
                disabled={saving || !addForm.firstName.trim() || !addForm.email.trim()}
              >
