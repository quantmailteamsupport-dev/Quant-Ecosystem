...
export const DrivePage: React.FC<DrivePageProps> = ({ userId }) => {
  ...
  const [quota, setQuota] = useState<StorageQuota>({ used: 0, total: 15 * 1024 * 1024 * 1024, breakdown: [] });
  ...
  const handleUpload = useCallback(async (fileList: FileList | null) => {
    ...
    const newUploads: UploadItem[] = Array.from(fileList).map((file, i) => ({
      id: `upload-${Date.now()}-${i}`, file, progress: 0, status: 'pending' as const
    }));
    ...
    for (const upload of newUploads) {
      ...
      try {
        ...
        setUploads(prev => prev.map(u => u.id === upload.id ? { ...u, status: 'complete' as const, progress: 100 } : u));
      } catch (err) {
        setUploads(prev => prev.map(u => u.id === upload.id ? { ...u, status: 'error' as const, error: err instanceof Error ? err.message : 'Upload failed' } : u));
      }
    }
    fetchFiles();
  ...
  const handleShare = useCallback(async () => {
    ...
    try {
      ...
      setFiles(prev => prev.map(f => f.id === showShareModal.id ? { ...f, sharedWith: [...f.sharedWith, { email: shareEmail, permission: sharePermission }] } : f));
      setShareEmail('');
    } catch (err) {
  ...
  const getFileIcon = (file: DriveFile): string => {
    ...
    if (file.mimeType.includes('spreadsheet') || file.mimeType.includes('excel')) return '\u{1F4CA}';
    return '\u{1F4C4}';
  };
  ...
  if (error && files.length === 0) {
    return (<div className="drive-error"><h2>Failed to Load Drive</h2><p>{error}</p><button onClick={fetchFiles}>Retry</button></div>);
  }

  return (
    <div className="drive-page" onDragOver={(e) => { e.preventDefault(); setIsDragOver(true); }} onDragLeave={() => setIsDragOver(false)} onDrop={(e) => { e.preventDefault(); setIsDragOver(false); handleUpload(e.dataTransfer.files); }} onClick={() => setContextMenu(null)}>
      {isDragOver && (<div className="upload-overlay"><div className="overlay-content"><span className="upload-icon">\u{2B06}</span><p>Drop files to upload</p></div></div>)}

      <header className="drive-header">
        <h1>Drive</h1>
        <div className="drive-actions">
          <button onClick={() => fileInputRef.current?.click()} className="upload-btn">Upload</button>
          <button onClick={() => setShowNewFolderDialog(true)} className="new-folder-btn">New Folder</button>
      ...
      <div className="drive-layout">
        <aside className="drive-sidebar">
          <nav className="folder-tree">
            <button onClick={() => { setCurrentFolderId(null); setBreadcrumbs([{ id: null, name: 'My Drive' }]); }} className="tree-item">My Drive</button>
          ...
          <div className="storage-quota">
            <div className="quota-bar"><div className="quota-fill" style={{ width: `${quotaPercentage}%` }}></div></div>
            <span className="quota-text">{formatFileSize(quota.used)} of {formatFileSize(quota.total)} used ({quotaPercentage}%)</span>
          </div>
        </aside>
        ...
        <main className="drive-content">
          <div className="breadcrumbs">
            {breadcrumbs.map((crumb, i) => (
              <span key={i}>
                <button onClick={() => navigateToBreadcrumb(i)} className="breadcrumb-link">{crumb.name}</button>
          ...
          {loading ? (
            <div className="drive-loading">{Array.from({ length: 8 }).map((_, i) => <div key={i} className="file-skeleton"></div>)}</div>
          ) : sortedFiles.length === 0 ? (
            <div className="empty-state"><h3>No files here</h3><p>Upload files or create a folder to get started.</p></div>
          ) : viewMode === 'grid' ? (
            <div className="file-grid">
              {sortedFiles.map(file => (
                <div key={file.id} className={`file-card ${selectedFiles.has(file.id) ? 'selected' : ''}`} onClick={() => file.type === 'folder' ? navigateToFolder(file) : setSelectedFiles(prev => { const n = new Set(prev); n.has(file.id) ? n.delete(file.id) : n.add(file.id); return n; })} onContextMenu={(e) => handleContextMenu(e, file)} onDoubleClick={() => file.type === 'folder' && navigateToFolder(file)}>
                  <div className="file-thumbnail">
                    {file.thumbnailUrl ? <img src={file.thumbnailUrl} alt="" /> : <span className="file-icon-large">{getFileIcon(file)}</span>}
                  </div>
                  <div className="file-card-info">
                    {renaming === file.id ? (
                      <input type="text" value={renameValue} onChange={(e) => setRenameValue(e.target.value)} onBlur={() => handleRename(file.id)} onKeyDown={(e) => { if (e.key === 'Enter') handleRename(file.id); }} autoFocus />
                    ) : (
            ...
            <table className="file-table">
              <thead><tr><th>Name</th><th>Modified</th><th>Size</th><th>Shared</th></tr></thead>
              <tbody>
                {sortedFiles.map(file => (
                  <tr key={file.id} className={selectedFiles.has(file.id) ? 'selected' : ''} onClick={() => file.type === 'folder' ? navigateToFolder(file) : setSelectedFiles(prev => { const n = new Set(prev); n.has(file.id) ? n.delete(file.id) : n.add(file.id); return n; })} onContextMenu={(e) => handleContextMenu(e, file)}>
                    <td className="file-name-cell"><span className="file-icon">{getFileIcon(file)}</span>{file.name}{file.isStarred && ' \u2B50'}</td>
        ...
        <div className="upload-panel">
          ...
          {uploads.map(u => (
            <div key={u.id} className={`upload-item ${u.status}`}>
              ...
              <div className="upload-progress-bar"><div className="progress-fill" style={{ width: `${u.progress}%` }}></div></div>
              <span className="upload-status">{u.status === 'complete' ? '\u2713' : u.status === 'error' ? '\u2717' : `${u.progress}%`}</span>
            </div>
          ))}
          <button onClick={() => setUploads(prev => prev.filter(u => u.status !== 'complete'))} className="clear-uploads">Clear completed</button>
        </div>
      )}

      {contextMenu && (
        <div className="context-menu" style={{ left: contextMenu.x, top: contextMenu.y }}>
          <button onClick={() => { setRenaming(contextMenu.file.id); setRenameValue(contextMenu.file.name); setContextMenu(null); }}>Rename</button>
          <button onClick={() => { setShowShareModal(contextMenu.file); setContextMenu(null); }}>Share</button>
          <button onClick={() => { setShowVersions(contextMenu.file); setContextMenu(null); }}>Version history</button>
          <button onClick={() => { handleStar(contextMenu.file.id); setContextMenu(null); }}>{contextMenu.file.isStarred ? 'Unstar' : 'Star'}</button>
          <button onClick={() => { handleDelete([contextMenu.file.id]); }}>Delete</button>
        </div>
      )}
          ...
          <div className="share-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="share-form">
              ...
              <select value={sharePermission} onChange={(e) => setSharePermission(e.target.value as 'view' | 'edit')}>
                <option value="view">Viewer</option><option value="edit">Editor</option>
              </select>
            ...
            <div className="shared-list">
              ...
              {showShareModal.sharedWith.map(s => (
                <div key={s.email} className="shared-item"><span>{s.email}</span><span className="perm-badge">{s.permission}</span></div>
              ))}
            </div>
        ...
        <div className="modal-overlay" onClick={() => setShowVersions(null)}>
          <div className="versions-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="versions-list">
              {showVersions.versions.length === 0 ? <p>No previous versions.</p> : showVersions.versions.map(v => (
      ...
      {showNewFolderDialog && (
        <div className="modal-overlay" onClick={() => setShowNewFolderDialog(false)}>
          <div className="new-folder-modal" onClick={(e) => e.stopPropagation()}>
            <h3>New Folder</h3>
            <input type="text" value={newFolderName} onChange={(e) => setNewFolderName(e.target.value)} placeholder="Folder name" autoFocus onKeyDown={(e) => { if (e.key === 'Enter') handleCreateFolder(); }} />
            <div className="modal-actions"><button onClick={() => setShowNewFolderDialog(false)}>Cancel</button><button onClick={handleCreateFolder} disabled={!newFolderName.trim()}>Create</button></div>
          </div>
        </div>
      )}
    </div>
