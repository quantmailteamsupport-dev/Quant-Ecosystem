// Quantmail - Gesture Service
// Mobile gesture handling for email platform
...
export class GestureService {
  ...
  private registerDefaultGestures(): void {
    this.registerGesture({ id: 'swipe_left_archive', type: 'swipe', priority: 10, enabled: true, options: { direction: 'left', action: 'archive' } });
    this.registerGesture({ id: 'swipe_right_mark_unread', type: 'swipe', priority: 10, enabled: true, options: { direction: 'right', action: 'mark_unread' } });
    this.registerGesture({ id: 'double_tap_star', type: 'double_tap', priority: 5, enabled: true, options: { action: 'star' } });
    this.registerGesture({ id: 'long_press_context_menu', type: 'long_press', priority: 8, enabled: true, options: { action: 'context_menu' } });
