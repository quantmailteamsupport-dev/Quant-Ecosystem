...
const VideoPlayer: React.FC<{
  ...
  onStateChange: (s: Partial<PlayerState>) => void;
}> = ({ video, playerState, onStateChange }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute inset-0 flex items-center justify-center transition-opacity ${playerState.showControls || !playerState.playing ? 'opacity-100' : 'opacity-0'}`}
      >
        <button
      ...
      <div
        className={`absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 to-transparent p-4 transition-opacity ${playerState.showControls || !playerState.playing ? 'opacity-100' : 'opacity-0'}`}
      >
        <div
          className="progress-bar h-1 bg-gray-600 rounded cursor-pointer mb-3 group/progress hover:h-2 transition-all"
          onClick={handleSeek}
        >
          <div
          ...
          <div
            className="h-full bg-gray-500 rounded absolute top-0"
            style={{ width: `${playerState.buffered}%`, opacity: 0.3 }}
          />
        </div>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => onStateChange({ muted: !playerState.muted })}
              className="text-white text-lg hover:text-gray-300"
            >
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => onStateChange({ pip: !playerState.pip })}
            ...
            <button
              onClick={() => onStateChange({ theaterMode: !playerState.theaterMode })}
              className="text-white text-sm hover:text-gray-300"
              title="Theater Mode"
            >
              🖥️
            ...
            <button
              onClick={() => onStateChange({ fullscreen: !playerState.fullscreen })}
              className="text-white text-sm hover:text-gray-300"
              title="Fullscreen"
            >
              ⛶
...
const ChapterTimeline: React.FC<{
  chapters: Chapter[];
  currentTime: number;
  onSeek: (time: number) => void;
}> = ({ chapters, currentTime, onSeek }) => {
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`text-sm px-3 py-1 rounded-full ${sortBy === 'top' ? 'bg-white text-black' : 'text-gray-400 hover:text-white'}`}
          >
            Top
          </button>
          <button
            onClick={() => setSortBy('newest')}
            className={`text-sm px-3 py-1 rounded-full ${sortBy === 'newest' ? 'bg-white text-black' : 'text-gray-400 hover:text-white'}`}
          >
            Newest
          </button>
        </div>
          ...
          <div className="space-y-4">
            {comments.map((comment) => (
              <div key={comment.id} className="flex gap-3">
                ...
                <div className="flex-1">
                  {comment.isPinned && (
                    <span className="text-xs text-gray-400 mb-1 block">📌 Pinned</span>
                  )}
...
export const WatchPage: React.FC<{ videoId?: string }> = ({ videoId = 'demo-video' }) => {
  ...
  useEffect(() => {
    ...
    const mockVideo: VideoData = {
      ...
      description:
        'In this comprehensive tutorial, we build a complete video streaming platform including upload, transcoding, playback, comments, and recommendations. This covers React, Node.js, FFmpeg, HLS streaming, and more.\n\nTimestamps:\n0:00 - Introduction\n5:30 - Setting up the project\n12:00 - Video upload system\n25:00 - Transcoding pipeline\n40:00 - HLS streaming setup\n55:00 - Player UI\n1:20:00 - Comments and interactions\n1:45:00 - Recommendations engine\n2:00:00 - Deployment',
  ...
  return (
    <div className="watch-page min-h-screen bg-gray-950 text-white pt-14">
      <div
        className={`flex ${playerState.theaterMode ? 'flex-col' : 'flex-col lg:flex-row'} gap-6 p-4 max-w-[1800px] mx-auto`}
      >
        <div className={`${playerState.theaterMode ? 'w-full' : 'flex-1'}`}>
          ...
          <ChapterTimeline
            ...
            onSeek={(time) => handlePlayerStateChange({ currentTime: time })}
          />
          <div className="video-info mt-4">
            ...
            <div className="flex flex-wrap items-center justify-between mt-3 gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">
                    {formatNumber(video.channelSubscribers)} subscribers
                  </p>
                </div>
                <button
                ...
                {subscribed && (
                  <button className="p-2 rounded-full hover:bg-gray-800" title="Notifications">
                    🔔
                  </button>
                )}
              </div>
              <div className="flex items-center gap-2">
                ...
                <button className="flex items-center gap-1 px-4 py-2 bg-gray-800 rounded-full text-sm hover:bg-gray-700">
                  ↗️ Share
                </button>
                <button className="flex items-center gap-1 px-4 py-2 bg-gray-800 rounded-full text-sm hover:bg-gray-700">
                  ⬇️ Download
            ...
            <div
              className={`description mt-4 p-3 bg-gray-800 rounded-xl ${descExpanded ? '' : 'max-h-24 overflow-hidden relative'}`}
            >
            ...
            <div className="space-y-3">
              {relatedVideos.map((rv) => (
                <a
                  key={rv.id}
                  href={`#/watch/${rv.id}`}
                  className="flex gap-2 group hover:bg-gray-800 rounded-lg p-1 transition-colors"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500">
                      {formatNumber(rv.views)} views · {formatTimeAgo(rv.uploadedAt)}
                    </p>
                  </div>
                </a>
              ))}
