// ============================================================================
// QuantTube API Server
// Video, Music, and Shows streaming platform with live streaming and creator tools
// ============================================================================

import type { Request, Response, NextFunction, Middleware } from './middleware';
import { RateLimiter, corsMiddleware, requestIdMiddleware, securityHeaders, loggingMiddleware, errorHandler, AppError } from './middleware';
...
import type { RouteDefinition } from './routes/videos';
...
const defaultConfig: ServerConfig = {
  port: 3005,
  host: '0.0.0.0',
  corsOrigins: ['https://tube.quant.app', 'https://neon.quant.app', 'https://mail.quant.app', 'http://localhost:3000', 'http://localhost:3005'],
...
class Router {
  ...
  register(routes: RouteDefinition[]): void {
    for (const route of routes) {
      ...
      this.routes.push({ method: route.method, pathPattern: pattern, paramNames, handler: route.handler, middleware: route.middleware || [], requiresAuth: route.requiresAuth ?? true });
    }
  }
  ...
  private pathToRegex(path: string): { pattern: RegExp; paramNames: string[] } {
    ...
    const regexStr = path.replace(/:([a-zA-Z0-9_]+)/g, (_, name) => { paramNames.push(name); return '([^/]+)'; });
    return { pattern: new RegExp(`^${regexStr}$`), paramNames };
  }
}

export class QuantTubeServer {
  ...
  private setupRoutes(): void {
    this.router.register(videoRoutes);
    this.router.register(musicRoutes);
    this.router.register(showRoutes);
    this.router.register(channelRoutes);
    this.router.register(playlistRoutes);
    this.router.register(interactionRoutes);
    this.router.register(liveRoutes);
    this.router.register(monetizationRoutes);
    this.router.register(aiRoutes);
    this.router.register(searchRoutes);
  }
  ...
  async handleRequest(req: Request, res: Response): Promise<void> {
    try {
      for (const mw of this.globalMiddleware) {
        let ok = true;
        await new Promise<void>(resolve => { mw(req, res, (err?: Error) => { if (err) ok = false; resolve(); }); });
      ...
      if (route.requiresAuth) {
        ...
        req.user = { id: decoded.sub, email: decoded.email || '', username: decoded.username || '', role: decoded.role || 'viewer', channelId: decoded.channelId };
      }
  ...
  getRoutes() {
    const allRoutes = [...videoRoutes, ...musicRoutes, ...showRoutes, ...channelRoutes, ...playlistRoutes, ...interactionRoutes, ...liveRoutes, ...monetizationRoutes, ...aiRoutes, ...searchRoutes];
    return allRoutes.map(r => ({ method: r.method, path: r.path, requiresAuth: r.requiresAuth ?? true }));
  }

  getHealthStatus() {
    return { status: 'healthy', service: 'quantube', uptime: process.uptime(), version: '1.0.0', routes: this.getRoutes().length, features: ['video-streaming', 'music', 'shows', 'live', 'creator-tools', 'ai-recommendations', 'monetization'] };
  }
  ...
  start(): void {
    ...
    server.listen(this.config.port, this.config.host, () => {
      console.log(`[QuantTube] Server running at http://${this.config.host}:${this.config.port}`);
      console.log(`[QuantTube] Routes registered: ${this.getRoutes().length}`);
    });
  }
}

export default QuantTubeServer;
