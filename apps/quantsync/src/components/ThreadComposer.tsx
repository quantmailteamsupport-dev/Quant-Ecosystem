...
const ThreadComposer: React.FC<ThreadComposerProps> = ({ maxChars = 5000, maxPosts = 25, onSubmit, onCancel, submitting = false }) => {
  ...
  const updateContent = useCallback((id: string, content: string) => {
    setPosts(prev => prev.map(p => p.id === id ? { ...p, content, charCount: content.length } : p));
    setError(null);
  }, []);
  ...
  const handleDrop = useCallback((e: React.DragEvent, targetId: string) => {
    ...
    setPosts(prev => {
      ...
      newPosts.splice(targetIdx, 0, dragged);
      return newPosts;
    });
    setDraggedId(null);
  }, [draggedId]);
    ...
    <div className="max-w-2xl mx-auto bg-white rounded-xl border shadow-sm">
      <div className="p-4 border-b flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500">{filledCount} posts · {totalChars} total characters</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => setShowPreview(!showPreview)} className={`px-3 py-1 rounded-full text-xs ${showPreview ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'}`}>
      ...
      {error && <div className="mx-4 mt-3 p-2 bg-red-50 border border-red-200 rounded-lg text-red-700 text-xs">{error}</div>}

      {showPreview ? (
        <div className="p-4 space-y-4">
          <h3 className="text-sm font-medium text-gray-500 mb-3">Thread Preview</h3>
          {posts.filter(p => p.content.trim()).map((post, idx) => (
        ...
        <div className="p-4 space-y-3">
          {posts.map((post, idx) => (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-2">
                  <span className={`text-xs ${post.charCount > maxChars ? 'text-red-500 font-bold' : 'text-gray-400'}`}>
                    {post.charCount}/{maxChars}
                  </span>
                  {posts.length > 2 && (
                    <button onClick={() => removePost(post.id)} className="w-6 h-6 flex items-center justify-center rounded-full hover:bg-red-50 text-red-400 hover:text-red-600 text-xs">✕</button>
                  )}
                </div>
              </div>
              <textarea
          ...
          {posts.length < maxPosts && (
            <button onClick={addPost} className="w-full py-3 border-2 border-dashed border-gray-200 rounded-xl text-sm text-gray-500 hover:border-blue-300 hover:text-blue-500 transition-colors">
      ...
      <div className="p-4 border-t flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={onCancel} className="px-4 py-2 border rounded-full text-sm font-medium">Cancel</button>
          <button onClick={handleSubmit} disabled={!canSubmit} className="px-4 py-2 bg-blue-500 text-white rounded-full text-sm font-medium disabled:opacity-50">
