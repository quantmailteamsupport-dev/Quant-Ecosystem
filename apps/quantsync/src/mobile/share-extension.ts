// Quantsync - Share Extension Service
// Mobile share extension for cloud storage platform
...
export class ShareExtensionService {
  ...
  private registerDefaultTargets(): void {
    const targets: ShareTarget[] = [
      { id: 'upload_file', contentTypes: ['text', 'url', 'image', 'file'], maxFileSize: 52428800, maxFiles: 10, description: 'Upload File' },
      { id: 'quick_save', contentTypes: ['text', 'url', 'image', 'video', 'file'], maxFileSize: 104857600, maxFiles: 20, description: 'Save to quantsync' },
    ];
  ...
  public async prepareCompose(content: SharedContent): Promise<ComposeData> {
    ...
    switch (content.type) {
      ...
      case 'file':
        ...
        compose.title = content.fileName || 'Upload File';
        break;
    }
    return compose;
  }
