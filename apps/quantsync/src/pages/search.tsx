...
interface SearchResult {
  posts: { id: string; authorName: string; authorHandle: string; authorAvatar: string; content: string; likes: number; createdAt: string }[];
  people: { id: string; name: string; handle: string; avatar: string; bio: string; isVerified: boolean; followersCount: number; isFollowing: boolean }[];
  communities: { id: string; name: string; icon: string; memberCount: number; description: string }[];
...
const SearchPage: React.FC = () => {
  ...
  const [results, setResults] = useState<SearchResult>({ posts: [], people: [], communities: [], hashtags: [] });
  ...
  const [filters, setFilters] = useState<SearchFilters>({ dateFrom: '', dateTo: '', fromUser: '', hasMedia: false, minLikes: 0 });
  ...
  const hasResults = results.posts.length > 0 || results.people.length > 0 || results.communities.length > 0 || results.hashtags.length > 0;
    ...
    <div className="max-w-2xl mx-auto min-h-screen">
      <header className="sticky top-0 bg-white/90 backdrop-blur border-b z-10 px-4 py-3">
        <div className="relative">
          ...
          {query && (
            <button onClick={() => { setQuery(''); setSuggestions([]); }} className="absolute right-3 top-2.5 text-gray-400 hover:text-gray-600">✕</button>
          )}
        </div>
          ...
          <div className="absolute left-4 right-4 top-16 bg-white border rounded-xl shadow-lg z-30 max-h-64 overflow-y-auto">
            {suggestions.length > 0 ? (
              suggestions.map((s, idx) => (
                <button key={idx} onClick={() => { setQuery(s); executeSearch(s); }} className="w-full text-left px-4 py-2 hover:bg-gray-50 text-sm flex items-center gap-2">
              ...
              <>
                <div className="flex items-center justify-between px-4 py-2 border-b">
                  ...
                  <button onClick={clearRecentSearches} className="text-xs text-blue-500">Clear all</button>
                </div>
        ...
        <div className="flex items-center justify-between mt-3">
          <div className="flex gap-1">
            {(['posts', 'people', 'communities', 'hashtags'] as SearchTab[]).map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)} className={`px-3 py-1 rounded-full text-xs capitalize ${activeTab === tab ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-600'}`}>
                {tab}
              </button>
            ))}
          </div>
          <button onClick={() => setShowFilters(!showFilters)} className={`text-xs px-2 py-1 rounded ${showFilters ? 'bg-blue-100 text-blue-700' : 'text-gray-500'}`}>
            Filters
          </button>
        </div>
          ...
          <div className="mt-3 p-3 bg-gray-50 rounded-xl space-y-2">
            <div className="flex gap-2">
              <div className="flex-1">
                ...
                <input type="date" value={filters.dateFrom} onChange={(e) => setFilters(f => ({ ...f, dateFrom: e.target.value }))} className="w-full border rounded px-2 py-1 text-xs" />
              </div>
              <div className="flex-1">
                <label className="text-xs text-gray-500">To date</label>
                <input type="date" value={filters.dateTo} onChange={(e) => setFilters(f => ({ ...f, dateTo: e.target.value }))} className="w-full border rounded px-2 py-1 text-xs" />
              </div>
            </div>
            <div>
              <label className="text-xs text-gray-500">From user</label>
              <input type="text" value={filters.fromUser} onChange={(e) => setFilters(f => ({ ...f, fromUser: e.target.value }))} className="w-full border rounded px-2 py-1 text-xs" placeholder="@handle" />
            </div>
            <div className="flex items-center gap-4">
              <label className="flex items-center gap-1 text-xs">
                <input type="checkbox" checked={filters.hasMedia} onChange={(e) => setFilters(f => ({ ...f, hasMedia: e.target.checked }))} />
                Has media
              </label>
              <div className="flex items-center gap-1">
                <label className="text-xs text-gray-500">Min likes:</label>
                <input type="number" value={filters.minLikes} onChange={(e) => setFilters(f => ({ ...f, minLikes: Number(e.target.value) }))} className="w-16 border rounded px-2 py-0.5 text-xs" min="0" />
              </div>
            </div>
          </div>
        )}
      ...
      {error && <div className="mx-4 mt-4 p-3 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm">{error}</div>}
            ...
            <div key={person.id} className="px-4 py-3 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <span className="text-xs text-gray-500">{person.followersCount.toLocaleString()} followers</span>
                </div>
              </div>
              <button onClick={() => handleFollow(person.id)} className={`px-4 py-1.5 rounded-full text-sm font-medium ${person.isFollowing ? 'border' : 'bg-gray-900 text-white'}`}>
      ...
      {activeTab === 'communities' && results.communities.length > 0 && (
        <div className="divide-y">
          {results.communities.map(community => (
            <div key={community.id} className="px-4 py-3 flex items-center gap-3 hover:bg-gray-50 cursor-pointer">
            ...
            <div key={hashtag.tag} className="px-4 py-3 hover:bg-gray-50 cursor-pointer">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500">{hashtag.postCount.toLocaleString()} posts</p>
                </div>
                {hashtag.trending && <span className="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">Trending</span>}
              </div>
            </div>
          ))}
        </div>
