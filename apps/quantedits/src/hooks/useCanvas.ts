...
export function useCanvas(
  ...
  const saveHistory = useCallback(
    (description: string) => {
      setUndoStack((prev) => [
        ...prev.slice(-49),
        { elements: JSON.parse(JSON.stringify(state.elements)), description },
      ]);
      setRedoStack([]);
    },
    [state.elements],
  ...
  const alignGuides = useMemo((): AlignGuide[] => {
    ...
    if (Math.abs(centerX - state.canvasWidth / 2) < 5)
      guides.push({ type: 'vertical', position: state.canvasWidth / 2, source: 'canvas-center' });
    if (Math.abs(centerY - state.canvasHeight / 2) < 5)
      guides.push({
        type: 'horizontal',
        position: state.canvasHeight / 2,
        source: 'canvas-center',
      });
    state.elements.forEach((el) => {
      if (el.id === sel.id) return;
      if (Math.abs(sel.x - el.x) < 5)
        guides.push({ type: 'vertical', position: el.x, source: el.id });
      if (Math.abs(sel.x + sel.width - (el.x + el.width)) < 5)
        guides.push({ type: 'vertical', position: el.x + el.width, source: el.id });
      if (Math.abs(sel.y - el.y) < 5)
        guides.push({ type: 'horizontal', position: el.y, source: el.id });
      if (Math.abs(sel.y + sel.height - (el.y + el.height)) < 5)
        guides.push({ type: 'horizontal', position: el.y + el.height, source: el.id });
    });
    return guides;
  ...
  const addElement = useCallback(
    (element: Omit<CanvasElement, 'id' | 'zIndex'>) => {
      ...
      setState((prev) => ({
        ...prev,
        elements: [...prev.elements, { ...element, id, zIndex }],
  ...
  const selectAll = useCallback(() => {
    setState((prev) => ({ ...prev, selectedIds: new Set(prev.elements.map((e) => e.id)) }));
  }, []);
  ...
  const resizeElement = useCallback((id: string, width: number, height: number) => {
    setState((prev) => ({
      ...prev,
      elements: prev.elements.map((e) =>
        e.id === id ? { ...e, width: Math.max(1, width), height: Math.max(1, height) } : e,
      ),
    }));
  }, []);
  ...
  const rotateElement = useCallback((id: string, rotation: number) => {
    setState((prev) => ({
      ...prev,
      elements: prev.elements.map((e) => (e.id === id ? { ...e, rotation: rotation % 360 } : e)),
    }));
  }, []);
  ...
  const groupSelected = useCallback(() => {
    ...
    const group: CanvasElement = {
      ...
      locked: false,
      visible: true,
      name: `Group ${state.elements.filter((e) => e.type === 'group').length + 1}`,
      zIndex: state.elements.length,
      fill: 'transparent',
      stroke: '',
      strokeWidth: 0,
    ...
    setState((prev) => ({
      ...prev,
      elements: [...prev.elements.filter((e) => !ids.includes(e.id)), group],
  ...
  const ungroupElement = useCallback(
    (id: string) => {
      ...
      setState((prev) => ({
        ...prev,
        elements: prev.elements.filter((e) => e.id !== id),
        selectedIds: new Set(group.groupChildren),
      }));
    },
    [state.elements, saveHistory],
  );
        ...
        elements: prev.elements.map((el) => {
          ...
          switch (alignment) {
            case 'left':
              return { ...el, x: 0 };
            case 'center':
              return { ...el, x: (prev.canvasWidth - el.width) / 2 };
            case 'right':
              return { ...el, x: prev.canvasWidth - el.width };
            case 'top':
              return { ...el, y: 0 };
            case 'middle':
              return { ...el, y: (prev.canvasHeight - el.height) / 2 };
            case 'bottom':
              return { ...el, y: prev.canvasHeight - el.height };
            default:
              return el;
          }
        }),
  ...
  const bringToFront = useCallback(
    (id: string) => {
      ...
      setState((prev) => ({
        ...prev,
        elements: prev.elements.map((e) => (e.id === id ? { ...e, zIndex: max + 1 } : e)),
      }));
    },
    [state.elements],
  );
  const sendToBack = useCallback((id: string) => {
    setState((prev) => ({
      ...prev,
      elements: prev.elements.map((e) =>
        e.id === id ? { ...e, zIndex: 0 } : e.zIndex >= 0 ? { ...e, zIndex: e.zIndex + 1 } : e,
      ),
    }));
  }, []);

  const copy = useCallback(() => {
    setClipboard(selectedElements.map((e) => ({ ...e })));
  }, [selectedElements]);
  const paste = useCallback(() => {
    ...
    const pasted = clipboard.map((e) => ({
      ...e,
      id: `el-paste-${Date.now()}-${Math.random().toString(36).slice(2)}`,
      x: e.x + 20,
      y: e.y + 20,
    }));
    setState((prev) => ({
      ...
      selectedIds: new Set(pasted.map((e) => e.id)),
    }));
  ...
  const cut = useCallback(() => {
    ...
    setState((prev) => ({
      ...prev,
      elements: prev.elements.filter((e) => !prev.selectedIds.has(e.id)),
      selectedIds: new Set(),
    }));
  }, [copy, saveHistory]);
  ...
  const undo = useCallback(() => {
    ...
    setRedoStack((r) => [
      ...r,
      { elements: JSON.parse(JSON.stringify(state.elements)), description: 'undo' },
    ]);
  ...
  const redo = useCallback(() => {
    ...
    setUndoStack((u) => [
      ...u,
      { elements: JSON.parse(JSON.stringify(state.elements)), description: 'redo' },
    ]);
