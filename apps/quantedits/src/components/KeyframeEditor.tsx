    ...
    <div className="keyframe-editor">
      <div className="keyframe-header">
        ...
        <div className="keyframe-controls">
          <button
            className="add-kf-btn"
            onClick={() => properties.length > 0 && handleAddKeyframeAtPlayhead(properties[0]!.id)}
            title="Add keyframe at playhead"
          >
            &#9670; +
          </button>
          <button
            className="remove-kf-btn"
            onClick={handleRemoveSelectedKeyframe}
          ...
          <button
            className={`graph-btn ${showGraph ? 'active' : ''}`}
      ...
      {selectedKeyframeData && (
        <div className="keyframe-details">
          <div className="detail-row">
            ...
            <input
              type="number"
              value={selectedKeyframeData.time.toFixed(2)}
              step={0.01}
              min={0}
              max={duration}
              onChange={(e) =>
                onUpdateKeyframe(selectedKeyframe!.propertyId, selectedKeyframe!.keyframeId, {
          ...
          <div className="detail-row">
            ...
            <input
              type="number"
              value={selectedKeyframeData.value.toFixed(2)}
              step={0.1}
              onChange={(e) =>
                onUpdateKeyframe(selectedKeyframe!.propertyId, selectedKeyframe!.keyframeId, {
          ...
          {showEasingPicker && (
            <div className="easing-picker">
              {EASING_OPTIONS.map((opt) => (
                <button
                  key={opt.type}
                  className={`easing-option ${selectedKeyframeData.easing === opt.type ? 'active' : ''}`}
                  onClick={() => handleEasingChange(opt.type)}
                >
          ...
          <div key={prop.id} className="keyframe-property">
            <div className="property-header">
              ...
              <button
                ...
                onClick={() => setGraphProperty(graphProperty === prop.id ? null : prop.id)}
              >
            ...
            <div className="keyframe-track" style={{ width: `${100 * zoom}%` }}>
              ...
              <button
                className="add-kf-at-playhead"
                style={{ left: `${(playhead / duration) * 100}%` }}
                onClick={() => handleAddKeyframeAtPlayhead(prop.id)}
        ...
        <div className="value-graph">
          ...
          <div className="graph-canvas">
            <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="graph-svg">
              ...
              />
              {properties
                .find((p) => p.id === graphProperty)
                ?.keyframes.map((kf) => {
                  const prop = properties.find((p) => p.id === graphProperty)!;
                  const x = (kf.time / duration) * 100;
                  const y =
                    (1 - (kf.value - prop.minValue) / (prop.maxValue - prop.minValue)) * 100;
                  return (
                    <circle
                      key={kf.id}
                      cx={x}
                      ...
                      onClick={() => handleKeyframeClick(graphProperty, kf.id)}
                    />
                  );
                })}
              <line
                x1={`${(playhead / duration) * 100}`}
                y1="0"
                x2={`${(playhead / duration) * 100}`}
                y2="100"
