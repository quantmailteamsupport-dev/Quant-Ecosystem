// Quantedits - Share Extension Service
// Mobile share extension for document editing platform
...
export class ShareExtensionService {
  ...
  private registerDefaultTargets(): void {
    const targets: ShareTarget[] = [
      { id: 'create_document', contentTypes: ['text', 'url', 'image', 'file'], maxFileSize: 52428800, maxFiles: 10, description: 'New Document' },
      { id: 'quick_save', contentTypes: ['text', 'url', 'image', 'video', 'file'], maxFileSize: 104857600, maxFiles: 20, description: 'Save to quantedits' },
    ];
  ...
  public async prepareCompose(content: SharedContent): Promise<ComposeData> {
    ...
    switch (content.type) {
      ...
      case 'file':
        ...
        compose.title = content.fileName || 'New Document';
        break;
    }
    return compose;
  }
