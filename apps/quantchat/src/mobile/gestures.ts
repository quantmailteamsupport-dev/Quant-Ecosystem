// Quantchat - Gesture Service
// Mobile gesture handling for messaging platform
...
export class GestureService {
  ...
  private registerDefaultGestures(): void {
    ...
    this.registerGesture({ id: 'swipe_right_reply', type: 'swipe', priority: 10, enabled: true, options: { direction: 'right', action: 'reply' } });
    this.registerGesture({ id: 'double_tap_like', type: 'double_tap', priority: 5, enabled: true, options: { action: 'like' } });
    this.registerGesture({ id: 'long_press_forward', type: 'long_press', priority: 8, enabled: true, options: { action: 'forward' } });
