// Quantchat - Share Extension Service
// Mobile share extension for messaging platform
...
export class ShareExtensionService {
  ...
  private registerDefaultTargets(): void {
    const targets: ShareTarget[] = [
      { id: 'send_to_chat', contentTypes: ['text', 'url', 'image', 'file'], maxFileSize: 52428800, maxFiles: 10, description: 'Send Message' },
      { id: 'quick_save', contentTypes: ['text', 'url', 'image', 'video', 'file'], maxFileSize: 104857600, maxFiles: 20, description: 'Save to quantchat' },
    ];
  ...
  public async prepareCompose(content: SharedContent): Promise<ComposeData> {
    ...
    switch (content.type) {
      ...
      case 'file':
        ...
        compose.title = content.fileName || 'Send Message';
        break;
    }
    return compose;
  }
