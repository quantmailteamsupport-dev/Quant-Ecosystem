// ============================================================================
// QuantNeon API Server
// Instagram-like social platform with mini games, AR/VR, and shopping
// ============================================================================
...
import { arVrRoutes } from './routes/ar-vr';
import { aiRoutes } from './routes/ai';
import { creatorRoutes } from './routes/creators';

import type { RouteDefinition } from './routes/posts';
...
const defaultConfig: ServerConfig = {
  port: 3006,
  host: '0.0.0.0',
  corsOrigins: ['https://neon.quant.app', 'https://tube.quant.app', 'https://mail.quant.app', 'http://localhost:3000', 'http://localhost:3006'],
...
export class QuantNeonServer {
  ...
  private setupRoutes(): void {
    ...
    this.router.register(reelRoutes);
    this.router.register(storyRoutes);
    this.router.register(profileRoutes);
    this.router.register(exploreRoutes);
    this.router.register(messageRoutes);
    this.router.register(shoppingRoutes);
    this.router.register(gameRoutes);
    this.router.register(arVrRoutes);
    this.router.register(aiRoutes);
    this.router.register(creatorRoutes);
  }
  ...
  async handleRequest(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (route.requiresAuth) {
        ...
        req.user = { id: decoded.sub, email: decoded.email || '', username: decoded.username || '', role: decoded.role || 'user', profileId: decoded.profileId };
      }
  ...
  getRoutes() {
    const allRoutes = [...postRoutes, ...reelRoutes, ...storyRoutes, ...profileRoutes, ...exploreRoutes, ...messageRoutes, ...shoppingRoutes, ...gameRoutes, ...arVrRoutes, ...aiRoutes, ...creatorRoutes];
    return allRoutes.map(r => ({ method: r.method, path: r.path, requiresAuth: r.requiresAuth ?? true }));
  }

  getHealthStatus() {
    return { status: 'healthy', service: 'quantneon', uptime: process.uptime(), version: '1.0.0', routes: this.getRoutes().length, features: ['posts', 'reels', 'stories', 'games', 'ar-vr', 'shopping', 'explore', 'creators'] };
  }
  ...
  start(): void {
    ...
    server.listen(this.config.port, this.config.host, () => {
      console.log(`[QuantNeon] Server running at http://${this.config.host}:${this.config.port}`);
      console.log(`[QuantNeon] Routes registered: ${this.getRoutes().length}`);
    });
  }
}

export default QuantNeonServer;
