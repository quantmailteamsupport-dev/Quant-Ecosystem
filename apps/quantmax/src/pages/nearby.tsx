...
const ALL_INTERESTS = [
  ...
  'Wine', 'Brunch', 'Sports', 'Fashion', 'Tech', 'Nature',
];
...
const NearbyPage: React.FC = () => {
  ...
  const loadNearbyPeople = useCallback(async () => {
    ...
    try {
      ...
      const mockPeople: NearbyPerson[] = Array.from({ length: 20 }, (_, i) => {
        ...
        return {
          id: `nearby-${i}`,
          displayName: ['Alex', 'Jordan', 'Sam', 'Morgan', 'Taylor', 'Casey', 'Riley', 'Avery', 'Quinn', 'Reese', 'Blake', 'Dakota', 'Finley', 'Harper', 'Kendall', 'Logan', 'Parker', 'Rowan', 'Skyler', 'Sage'][i],
          age: 20 + Math.floor(Math.random() * 15),
          distance: (i + 1) * 0.5 + Math.random() * 2,
          avatarUrl: `https://cdn.quantmax.app/nearby/avatars/${i}.jpg`,
          bio: [`Love exploring new places`, `Music is my life`, `Here for good vibes`, `Looking for adventure buddies`, `Coffee enthusiast`][i % 5],
  ...
  const loadWaves = useCallback(async () => {
    const mockWaves: WaveNotification[] = Array.from({ length: 5 }, (_, i) => ({
      ...
      fromAge: 22 + i,
      message: ['Hey! Noticed we both like music', 'Your profile is cool!', 'Hi neighbor!', 'We have so much in common!', 'Want to grab coffee?'][i],
  ...
  const handleSendWave = useCallback(() => {
    ...
    setPeople(prev => prev.map(p =>
      p.id === wavingTo ? { ...p, hasWaved: true } : p
    ));
    setWavingTo(null);
    setWaveMessage('');
  }, [wavingTo]);
  ...
  const handleAcceptWave = useCallback((waveId: string) => {
    setWaves(prev => prev.map(w =>
      w.id === waveId ? { ...w, status: 'accepted' } : w
    ));
  }, []);

  const handleDeclineWave = useCallback((waveId: string) => {
    setWaves(prev => prev.map(w =>
      w.id === waveId ? { ...w, status: 'declined' } : w
    ));
  }, []);
  ...
  const filteredPeople = useMemo(() => {
    let result = people.filter(p => {
      ...
      if (filters.interests.length > 0 && !filters.interests.some(fi => p.interests.includes(fi))) return false;
      return true;
    });
  ...
  if (error) {
    return (
      <div className="nearby-error">
        ...
        <button className="retry-btn" onClick={loadNearbyPeople}>Retry</button>
      </div>
    );
  }
      ...
      {showFilters && (
        <div className="filters-panel">
          ...
          <div className="filter-group">
            <label className="filter-label">Age Range: {filters.ageMin} - {filters.ageMax}</label>
            <div className="dual-range">
              <input type="range" min="18" max="65" value={filters.ageMin} onChange={(e) => setFilters(prev => ({ ...prev, ageMin: Math.min(Number(e.target.value), prev.ageMax - 1) }))} />
              <input type="range" min="18" max="65" value={filters.ageMax} onChange={(e) => setFilters(prev => ({ ...prev, ageMax: Math.max(Number(e.target.value), prev.ageMin + 1) }))} />
            </div>
          </div>

          <div className="filter-group">
            <label className="filter-label">Max Distance: {filters.maxDistance} km</label>
            <input type="range" min="1" max="100" value={filters.maxDistance} onChange={(e) => setFilters(prev => ({ ...prev, maxDistance: Number(e.target.value) }))} />
          </div>
          ...
          <div className="filter-toggles">
            <label className="toggle-label">
              <input type="checkbox" checked={filters.onlineOnly} onChange={(e) => setFilters(prev => ({ ...prev, onlineOnly: e.target.checked }))} />
              Online only
            </label>
            <label className="toggle-label">
              <input type="checkbox" checked={filters.verifiedOnly} onChange={(e) => setFilters(prev => ({ ...prev, verifiedOnly: e.target.checked }))} />
              Verified only
            </label>
          </div>
          ...
          {pendingWaves.map(wave => (
            <div key={wave.id} className="wave-item">
              ...
              <div className="wave-content">
                <span className="wave-name">{wave.fromUsername}, {wave.fromAge}</span>
                <p className="wave-message">{wave.message}</p>
                <span className="wave-time">{wave.timestamp}</span>
              </div>
              <div className="wave-actions">
                <button className="accept-wave-btn" onClick={() => handleAcceptWave(wave.id)}>Accept</button>
                <button className="decline-wave-btn" onClick={() => handleDeclineWave(wave.id)}>Decline</button>
              </div>
            </div>
          ))}
        </div>
          ...
          <div key={person.id} className="nearby-card">
            ...
            <div className="card-center">
              <div className="person-name-row">
                <h3 className="person-name">{person.displayName}, {person.age}</h3>
              ...
              {person.mutualInterests.length > 0 && (
                <div className="mutual-interests">
                  {person.mutualInterests.map(mi => (
                    <span key={mi} className="mutual-badge">{mi}</span>
                  ))}
                </div>
              )}
              <span className="last-active">{person.isOnline ? 'Online now' : person.lastActive}</span>
            </div>
            <div className="card-right">
              {person.hasWaved ? (
                <span className="waved-label">Waved</span>
              ) : person.receivedWave ? (
                <button className="wave-back-btn" onClick={() => handleWave(person.id)}>Wave Back</button>
              ) : (
        ...
        <div className="wave-modal-overlay" onClick={() => setWavingTo(null)}>
          <div className="wave-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="wave-modal-actions">
              <button className="cancel-btn" onClick={() => setWavingTo(null)}>Cancel</button>
              <button className="send-wave-btn" onClick={handleSendWave}>Send Wave</button>
            </div>
          </div>
        </div>
      )}
