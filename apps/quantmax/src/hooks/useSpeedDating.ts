...
export function useSpeedDating(userId: string): UseSpeedDatingReturn {
  const [session, setSession] = useState<SpeedDateSession>({
    id: '', status: 'waiting', partnerId: null, partnerName: null, partnerAvatar: null,
    timeRemaining: 180, extensions: 0, maxExtensions: 2, startedAt: null,
  });
  ...
  const joinQueue = useCallback(() => {
    ...
    matchRef.current = setTimeout(() => {
      ...
      setSession(prev => ({
        ...prev, status: 'active', partnerId, partnerName, partnerAvatar: `/avatars/${partnerId}.jpg`,
        timeRemaining: 180, startedAt: Date.now(), extensions: 0,
      }));
      setIsInQueue(false);
  ...
  const extendTime = useCallback((): boolean => {
    ...
    setSession(prev => ({ ...prev, timeRemaining: prev.timeRemaining + 60, extensions: prev.extensions + 1 }));
    return true;
  ...
  const ratePartner = useCallback((rating: 'like' | 'pass') => {
    ...
    if (rating === 'like' && Math.random() > 0.5) {
      const match: SpeedDateMatch = { partnerId: session.partnerId, partnerName: session.partnerName || '', partnerAvatar: session.partnerAvatar || '', matchedAt: Date.now(), mutual: true };
      setMatches(prev => [...prev, match]);
    }
    setSession(prev => ({ ...prev, status: 'completed', partnerId: null, partnerName: null, partnerAvatar: null }));
  }, [session]);

  const addContact = useCallback(() => {
    if (session.partnerId) {
      console.log(`Adding contact: ${session.partnerId}`);
    }
  }, [session.partnerId]);

  return { session, matches, ratings, isInQueue, isConnected: session.status === 'active', joinQueue, leaveQueue, extendTime, endDate, ratePartner, addContact };
}
