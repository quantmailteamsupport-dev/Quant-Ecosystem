// Quantmax - Gesture Service
// Mobile gesture handling for project management platform
...
export class GestureService {
  ...
  private registerDefaultGestures(): void {
    this.registerGesture({ id: 'swipe_left_complete_task', type: 'swipe', priority: 10, enabled: true, options: { direction: 'left', action: 'complete_task' } });
    this.registerGesture({ id: 'swipe_right_reassign', type: 'swipe', priority: 10, enabled: true, options: { direction: 'right', action: 'reassign' } });
    this.registerGesture({ id: 'double_tap_mark_priority', type: 'double_tap', priority: 5, enabled: true, options: { action: 'mark_priority' } });
    this.registerGesture({ id: 'long_press_add_subtask', type: 'long_press', priority: 8, enabled: true, options: { action: 'add_subtask' } });
