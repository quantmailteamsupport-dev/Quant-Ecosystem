// Quantmax - Share Extension Service
// Mobile share extension for project management platform
...
export class ShareExtensionService {
  ...
  private registerDefaultTargets(): void {
    const targets: ShareTarget[] = [
      { id: 'new_task', contentTypes: ['text', 'url', 'image', 'file'], maxFileSize: 52428800, maxFiles: 10, description: 'Create Task' },
      { id: 'quick_save', contentTypes: ['text', 'url', 'image', 'video', 'file'], maxFileSize: 104857600, maxFiles: 20, description: 'Save to quantmax' },
    ];
  ...
  public async prepareCompose(content: SharedContent): Promise<ComposeData> {
    ...
    switch (content.type) {
      ...
      case 'file':
        ...
        compose.title = content.fileName || 'Create Task';
        break;
    }
    return compose;
  }
