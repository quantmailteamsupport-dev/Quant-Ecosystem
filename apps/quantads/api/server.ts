// ============================================================================
// QuantAds API Server
// Ecosystem-wide advertising platform with real-time bidding
// ============================================================================
...
import type { RouteDefinition } from './routes/campaigns';
...
const defaultConfig: ServerConfig = {
  port: 3004,
  host: '0.0.0.0',
  corsOrigins: ['https://ads.quant.app', 'https://sync.quant.app', 'https://mail.quant.app', 'http://localhost:3000', 'http://localhost:3004'],
  rateLimit: { windowMs: 15 * 60 * 1000, maxRequests: 5000 },
...
export class QuantAdsServer {
  ...
  private setupRoutes(): void {
    this.router.register(campaignRoutes);
    this.router.register(creativeRoutes);
    this.router.register(targetingRoutes);
    this.router.register(biddingRoutes);
    this.router.register(analyticsRoutes);
    this.router.register(placementRoutes);
    this.router.register(billingRoutes);
    this.router.register(aiRoutes);
    this.router.register(policyRoutes);
  }
  ...
  async handleRequest(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (route.requiresAuth) {
        ...
        req.user = { id: decoded.sub, email: decoded.email || '', username: decoded.username || '', role: decoded.role || 'advertiser', advertiserId: decoded.advertiserId || decoded.sub };
      }
  ...
  getRoutes() {
    const allRoutes = [...campaignRoutes, ...creativeRoutes, ...targetingRoutes, ...biddingRoutes, ...analyticsRoutes, ...placementRoutes, ...billingRoutes, ...aiRoutes, ...policyRoutes];
    return allRoutes.map(r => ({ method: r.method, path: r.path, requiresAuth: r.requiresAuth ?? true }));
  }

  getHealthStatus() {
    return { status: 'healthy', service: 'quantads', uptime: process.uptime(), version: '1.0.0', routes: this.getRoutes().length, features: ['campaigns', 'rtb-auction', 'targeting', 'analytics', 'ai-optimization', 'cross-app-delivery'] };
  }
  ...
  start(): void {
    ...
    server.listen(this.config.port, this.config.host, () => {
      console.log(`[QuantAds] Server running at http://${this.config.host}:${this.config.port}`);
      console.log(`[QuantAds] Routes registered: ${this.getRoutes().length}`);
    });
  }
}

export default QuantAdsServer;
