...
export function AnalyticsChart({ title, type, data, color = '#4F46E5', showTrend = true, height = 300 }: AnalyticsChartProps) {
  ...
  const secondHalf = data.slice(midpoint).reduce((s, d) => s + d.value, 0) / ((data.length - midpoint) || 1);
  ...
  return {
    ...
    children: [
      { type: 'div', className: 'chart-header', children: [
        { type: 'h4', className: 'chart-title', text: title },
        showTrend && { type: 'span', className: `chart-trend ${trendDirection}`, text: `${trendPercent >= 0 ? '+' : ''}${trendPercent.toFixed(1)}%` },
      ].filter(Boolean) },
      { type: 'div', className: 'chart-stats', children: [
        { type: 'span', className: 'stat-total', text: `Total: ${formatValue(total)}` },
        { type: 'span', className: 'stat-avg', text: `Avg: ${formatValue(avg)}` },
      ] },
      { type: 'div', className: 'chart-canvas', children: data.map((point, i) => ({
        type: 'div',
        className: 'chart-bar',
        style: { height: `${(point.value / maxValue) * 100}%`, backgroundColor: color, left: `${(i / data.length) * 100}%` },
        title: `${point.date}: ${formatValue(point.value)}`,
      })) },
      { type: 'div', className: 'chart-x-axis', children: [
