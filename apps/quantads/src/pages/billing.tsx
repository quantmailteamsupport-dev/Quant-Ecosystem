...
const BillingPage: React.FC<BillingPageProps> = ({ accountId: _accountId }) => {
  ...
  const [autoReload, setAutoReload] = useState<AutoReloadConfig>({
    ...
    threshold: 100,
    reloadAmount: 500,
    maxReloadsPerMonth: 5,
    currentReloads: 0,
  });
  const [creditBalance, setCreditBalance] = useState<number>(0);
  const [monthlySpend, setMonthlySpend] = useState<number>(0);
  const [, setShowAddPayment] = useState<boolean>(false);
  const [showAddAlert, setShowAddAlert] = useState<boolean>(false);
  const [activeTab, setActiveTab] = useState<'overview' | 'invoices' | 'methods' | 'alerts'>(
    'overview',
  );
  ...
  const updateAutoReload = useCallback(
    async (field: keyof AutoReloadConfig, value: number | boolean) => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-white rounded-xl shadow-sm border p-5">
          ...
          <p className="text-2xl font-bold text-gray-900">
            {paymentMethods.find((p) => p.isDefault)?.brand || 'None'} ****
      ...
      <div className="flex gap-2 mb-6 border-b">
        {(['overview', 'invoices', 'methods', 'alerts'] as const).map((tab) => (
          <button
            ...
            className={`px-4 py-2 text-sm font-medium capitalize border-b-2 ${activeTab === tab ? 'border-green-500 text-green-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
          >
            {tab}
          </button>
        ))}
              ...
              {spendingData.slice(-30).map((point, idx) => {
                ...
                return (
                  <div key={idx} className="flex-1 flex flex-col items-center justify-end h-full">
                    <div
                      className="w-full bg-green-400 rounded-t hover:bg-green-600 transition-colors"
          ...
          <div className="bg-white rounded-xl shadow-sm border p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                ...
                className={`relative w-12 h-6 rounded-full transition-colors ${autoReload.enabled ? 'bg-green-500' : 'bg-gray-300'}`}
              >
                <div
                  className={`absolute w-5 h-5 bg-white rounded-full top-0.5 transition-transform ${autoReload.enabled ? 'translate-x-6' : 'translate-x-0.5'}`}
                />
              </button>
            </div>
            {autoReload.enabled && (
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => updateAutoReload('threshold', parseInt(e.target.value))}
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={autoReload.reloadAmount}
                    onChange={(e) => updateAutoReload('reloadAmount', parseInt(e.target.value))}
                    className="w-full px-3 py-2 border rounded-lg"
                  />
                </div>
              </div>
                ...
                {invoices.map((invoice) => (
                  <tr key={invoice.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs ${getInvoiceStatusColor(invoice.status)}`}
                      >
                        {invoice.status}
                      </span>
                    </td>
      ...
      {activeTab === 'methods' && (
        <div className="space-y-4">
          {paymentMethods.map((pm) => (
            <div
              key={pm.id}
              className={`bg-white rounded-xl shadow-sm border p-4 flex items-center justify-between ${pm.isDefault ? 'ring-2 ring-green-500' : ''}`}
            >
          ...
          <button
            ...
            className="w-full py-3 border-2 border-dashed border-gray-300 rounded-xl text-gray-500 hover:border-green-500 hover:text-green-600"
          >
            + Add Payment Method
          </button>
        </div>
      ...
      {activeTab === 'alerts' && (
        <div className="space-y-4">
          {alerts.map((alert) => (
            <div
              key={alert.id}
              className={`bg-white rounded-xl shadow-sm border p-4 ${alert.isTriggered ? 'border-red-300 bg-red-50' : ''}`}
            >
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-medium capitalize">{alert.type.replace('_', ' ')}</p>
                  <p className="text-sm text-gray-600">
                    Threshold: {formatCurrency(alert.threshold)} | Current:{' '}
          ...
          {showAddAlert ? (
            <div className="bg-white rounded-xl shadow-sm border p-4">
              <div className="flex items-center gap-4">
                ...
                />
                <button
                  onClick={addBudgetAlert}
                  disabled={saving}
                  className="px-4 py-2 bg-green-500 text-white rounded-lg"
                >
                  {saving ? 'Adding...' : 'Add Alert'}
                </button>
                <button onClick={() => setShowAddAlert(false)} className="text-gray-500">
                  Cancel
                </button>
              </div>
            </div>
