...
interface PixelEvent {
  id: string;
  pixelId: string;
  type: 'PageView' | 'AddToCart' | 'Purchase' | 'Lead' | 'ViewContent' | 'InitiateCheckout' | 'CompleteRegistration' | 'Search' | 'AddPaymentInfo' | 'Subscribe';
  url: string;
  value?: number;
  currency?: string;
  timestamp: string;
...
const EVENT_TYPES = ['PageView', 'AddToCart', 'Purchase', 'Lead', 'ViewContent', 'InitiateCheckout', 'CompleteRegistration', 'Search', 'AddPaymentInfo', 'Subscribe'] as const;

const PixelsPage: React.FC<PixelsPageProps> = ({ accountId }) => {
  const [pixels, setPixels] = useState<Pixel[]>([]);
  const [selectedPixel, setSelectedPixel] = useState<Pixel | null>(null);
  const [events, setEvents] = useState<PixelEvent[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  const [activeTab, setActiveTab] = useState<'overview' | 'events' | 'install' | 'test' | 'attribution'>('overview');
  ...
  const runTestEvents = useCallback(async () => {
    ...
    for (const eventType of EVENT_TYPES.slice(0, 5)) {
      ...
      try {
        ...
        setTestEvents(prev => [...prev, { name: eventType, status: 'failed', timestamp: new Date().toISOString(), details: 'Network error' }]);
      }
    }
    setTesting(false);
  }, [selectedPixel]);
  ...
  if (error && pixels.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-4">
        ...
        <button onClick={fetchPixels} className="px-6 py-2 bg-indigo-500 text-white rounded-lg">Retry</button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 min-h-screen">
      <header className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setShowCreatePixel(true)} className="px-4 py-2 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600">+ New Pixel</button>
      </header>
      ...
      {showCreatePixel && (
        <div className="bg-white rounded-xl shadow-lg border p-6 mb-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <input type="text" value={newPixelName} onChange={e => setNewPixelName(e.target.value)} placeholder="Pixel name..." className="px-3 py-2 border rounded-lg" />
            <input type="text" value={newPixelDomain} onChange={e => setNewPixelDomain(e.target.value)} placeholder="Domain (e.g., example.com)" className="px-3 py-2 border rounded-lg" />
          </div>
          <div className="flex gap-2 mt-4">
            <button onClick={createPixel} className="px-4 py-2 bg-indigo-500 text-white rounded-lg">Create</button>
            <button onClick={() => setShowCreatePixel(false)} className="px-4 py-2 text-gray-600">Cancel</button>
          </div>
        </div>
      )}
      ...
      {pixels.length > 0 && (
        <div className="flex gap-2 mb-6 overflow-x-auto">
          {pixels.map(pixel => (
            <button key={pixel.id} onClick={() => setSelectedPixel(pixel)} className={`flex-shrink-0 px-4 py-2 rounded-lg border ${selectedPixel?.id === pixel.id ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200 hover:bg-gray-50'}`}>
        ...
        <>
          <div className="flex gap-2 mb-6 border-b">
            {(['overview', 'events', 'install', 'test', 'attribution'] as const).map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-2 text-sm font-medium capitalize border-b-2 ${activeTab === tab ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500'}`}>{tab}</button>
            ))}
          </div>
          ...
          {activeTab === 'overview' && (
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="bg-white rounded-xl shadow-sm border p-4">
                ...
                <span className={`px-2 py-0.5 rounded-full text-xs mt-1 inline-block ${getStatusColor(selectedPixel.status)}`}>{selectedPixel.status}</span>
              </div>
              <div className="bg-white rounded-xl shadow-sm border p-4">
              ...
              <div className="bg-white rounded-xl shadow-sm border p-4">
                ...
                <p className="text-sm font-medium">{new Date(selectedPixel.lastActivity).toLocaleString()}</p>
              </div>
            </div>
          )}
                  ...
                  <table className="w-full">
                    <thead className="bg-gray-50 border-b">
                      <tr>
                        ...
                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500">Attribution</th>
                    ...
                    <tbody className="divide-y">
                      {events.map(event => (
                        <tr key={event.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 text-sm text-gray-600 truncate max-w-xs">{event.url}</td>
                          <td className="px-4 py-3 text-sm">{event.value ? `$${event.value}` : '-'}</td>
                          <td className="px-4 py-3">{event.attributed ? <span className="text-green-600 text-xs">Attributed</span> : <span className="text-gray-400 text-xs">Organic</span>}</td>
                          <td className="px-4 py-3 text-xs text-gray-500">{new Date(event.timestamp).toLocaleString()}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          {activeTab === 'install' && (
            <div className="bg-white rounded-xl shadow-sm border p-6">
              ...
              <p className="text-gray-600 mb-4">Add this code to the &lt;head&gt; section of every page on your website.</p>
              <div className="relative">
                <pre className="bg-gray-900 text-green-400 rounded-lg p-4 overflow-x-auto text-sm">{getInstallCode()}</pre>
                <button onClick={copyToClipboard} className="absolute top-2 right-2 px-3 py-1 bg-white text-gray-700 rounded text-sm hover:bg-gray-100">
                  {copied ? 'Copied!' : 'Copy'}
                </button>
              </div>
              <div className="mt-6">
          ...
          {activeTab === 'test' && (
            <div className="bg-white rounded-xl shadow-sm border p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={runTestEvents} disabled={testing} className="px-4 py-2 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 disabled:opacity-50">
                  {testing ? 'Testing...' : 'Run Test'}
                </button>
              </div>
              <div className="space-y-3">
                {testEvents.map((test, idx) => (
                  <div key={idx} className={`flex items-center justify-between p-3 rounded-lg ${test.status === 'success' ? 'bg-green-50' : test.status === 'failed' ? 'bg-red-50' : 'bg-gray-50'}`}>
                    <div className="flex items-center gap-2">
                      <span>{test.status === 'success' ? '✅' : test.status === 'failed' ? '❌' : '⏳'}</span>
                ...
                {testEvents.length === 0 && !testing && (
                  <p className="text-center text-gray-500 py-8">Click "Run Test" to verify your pixel installation</p>
                )}
              </div>
            </div>
          )}
                  ...
                  {attributionModels.map((model, idx) => (
                    <div key={idx} className="border rounded-lg p-4">
                      ...
                      <div className="mt-3 space-y-1">
                        <div className="flex justify-between text-sm"><span>Conversions</span><span className="font-medium">{model.conversions}</span></div>
                        <div className="flex justify-between text-sm"><span>Revenue</span><span className="font-medium">${model.revenue.toLocaleString()}</span></div>
                      </div>
                    </div>
                  ))}
                </div>
