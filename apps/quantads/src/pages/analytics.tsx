        ...
        <div className="flex items-center gap-2">
          {(['7d', '14d', '30d', '90d'] as DateRange[]).map((range) => (
            <button
              ...
              className={`px-3 py-1 rounded-lg text-sm ${dateRange === range ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
            >
              {range}
            </button>
          ))}
          ...
          <div className="flex gap-2">
            {['spend', 'impressions', 'clicks', 'conversions', 'ctr', 'roas'].map((metric) => (
              <button
                ...
                className={`px-3 py-1 rounded text-xs capitalize ${selectedMetric === metric ? 'bg-blue-100 text-blue-700 font-medium' : 'text-gray-500 hover:bg-gray-50'}`}
              >
                {metric}
              </button>
            ))}
        ...
        <div className="h-64 flex items-end gap-1 border-b border-l relative">
          {data.timeSeries.map((point, idx) => {
            ...
            return (
              ...
              >
                <div
                  className={`w-full rounded-t transition-all ${chartHover === idx ? 'bg-blue-600' : 'bg-blue-400'}`}
                ...
                {idx < data.funnel.length - 1 && (
                  <div className="flex justify-center py-1">
                    <span className="text-xs text-gray-400">
                      ↓{' '}
                      {data.funnel[idx + 1]
                        ? ((data.funnel[idx + 1]!.value / stage.value) * 100).toFixed(1)
                        : 0}
                      %
                    </span>
                  </div>
            ...
            <div className="flex gap-1 ml-auto">
              {(['placement', 'device', 'geo'] as BreakdownTab[]).map((tab) => (
                <button
                  ...
                  className={`px-3 py-1 rounded text-xs capitalize ${breakdownTab === tab ? 'bg-blue-100 text-blue-700' : 'text-gray-500 hover:bg-gray-50'}`}
                >
                  {tab}
                </button>
              ))}
