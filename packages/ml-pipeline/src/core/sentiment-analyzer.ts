...
export class SentimentAnalyzer {
  ...
  private initializeLexicons(): void {
    ...
    const positiveWords: [string, number][] = [
      ...
      ['fantastic', 0.9], ['outstanding', 0.95], ['superb', 0.9], ['love', 0.8], ['happy', 0.7],
      ['best', 0.85], ['perfect', 0.95], ['beautiful', 0.75], ['brilliant', 0.85], ['awesome', 0.85],
      ['enjoy', 0.65], ['pleased', 0.7], ['delighted', 0.8], ['impressive', 0.75], ['remarkable', 0.8],
      ['positive', 0.6], ['success', 0.7], ['win', 0.7], ['gain', 0.6], ['improve', 0.65],
      ['recommend', 0.7], ['satisfied', 0.7], ['helpful', 0.65], ['effective', 0.7], ['valuable', 0.7],
      ['exciting', 0.75], ['innovative', 0.7], ['reliable', 0.65], ['friendly', 0.6], ['comfortable', 0.6],
      ['convenient', 0.6], ['efficient', 0.65], ['elegant', 0.7], ['fun', 0.65], ['generous', 0.7],
    ];
    ...
    const negativeWords: [string, number][] = [
      ['bad', -0.6], ['terrible', -0.9], ['horrible', -0.9], ['awful', -0.85], ['poor', -0.6],
      ['worst', -0.95], ['hate', -0.85], ['ugly', -0.7], ['disgusting', -0.9], ['dreadful', -0.85],
      ['fail', -0.7], ['failure', -0.75], ['useless', -0.8], ['disappointing', -0.75], ['boring', -0.6],
      ['annoying', -0.65], ['frustrating', -0.7], ['pathetic', -0.8], ['mediocre', -0.5], ['inferior', -0.7],
      ['negative', -0.5], ['loss', -0.6], ['lose', -0.6], ['decline', -0.55], ['damage', -0.65],
      ['broken', -0.7], ['painful', -0.7], ['waste', -0.65], ['expensive', -0.4], ['slow', -0.4],
      ['confusing', -0.55], ['complicated', -0.45], ['unreliable', -0.7], ['rude', -0.7], ['unfair', -0.65],
      ['dangerous', -0.7], ['weak', -0.55], ['error', -0.6], ['bug', -0.5], ['crash', -0.7],
    ];
    ...
    const intensifierWords: [string, number][] = [
      ['very', 1.5], ['extremely', 2.0], ['incredibly', 1.8], ['absolutely', 2.0],
      ['completely', 1.7], ['totally', 1.7], ['really', 1.4], ['highly', 1.5],
      ['so', 1.3], ['truly', 1.5], ['deeply', 1.6], ['utterly', 1.8],
    ...
    const diminisherWords: [string, number][] = [
      ...
      ['a bit', 0.5], ['a little', 0.5], ['kind of', 0.6], ['sort of', 0.6],
    ];
    ...
    const comparativeWords: [string, number][] = [
      ['better', 0.5], ['worse', -0.5], ['superior', 0.6], ['inferior', -0.6],
      ['improved', 0.5], ['degraded', -0.5], ['faster', 0.4], ['slower', -0.4],
    ];
  ...
  analyze(text: string): SentimentResult {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (wordScore !== 0) {
        ...
        if (negationActive && i > 0) {
          const lastNegIdx = tokens.slice(0, i).lastIndexOf(
            tokens.find(t => this.negators.has(t.toLowerCase())) ?? ''
          );
          if (i - lastNegIdx > 3) {
            negationActive = false;
          }
    ...
    const normalizedScore = wordCount > 0
      ? Math.max(-1, Math.min(1, totalScore / Math.sqrt(wordCount)))
      : 0;
    const sentiment: SentimentLabel = normalizedScore > 0.1
      ? 'positive'
      : normalizedScore < -0.1
        ? 'negative'
        : 'neutral';
  ...
  analyzeAspects(text: string, aspects: string[]): SentimentResult {
    ...
    for (const aspect of aspects) {
      ...
      if (found) {
        const sentiment: SentimentLabel = aspectScore > 0.1 ? 'positive' : aspectScore < -0.1 ? 'negative' : 'neutral';
  ...
  aggregateSentiment(texts: string[]): SentimentResult {
    ...
    const sentiment: SentimentLabel = avgScore > 0.1 ? 'positive' : avgScore < -0.1 ? 'negative' : 'neutral';
