...
export class TaxService {
  ...
  private exemptions: Map<string, { customerId: string; jurisdiction: string; type: TaxType; validUntil: number }>;
  ...
  async calculateTax(params: {
    ...
    const { amount, sellerCountry, buyerCountry, buyerState, customerId, isDigitalService } = params;
  ...
  async validateTaxId(taxId: string, country: string): Promise<{ valid: boolean; type: string; name?: string; address?: string }> {
    ...
    const typeMap: Record<string, string> = {
      GB: 'VAT', DE: 'VAT', FR: 'VAT', IT: 'VAT', ES: 'VAT',
      IN: 'GST', AU: 'ABN', US: 'EIN', CA: 'GST',
    };
  ...
  async generateTaxReport(jurisdiction: string, startDate: number, endDate: number): Promise<TaxReport> {
  ...
  async applyExemptions(customerId: string, jurisdiction: string, type: TaxType, validUntilDays: number = 365): Promise<void> {
  ...
  async getReverseCharge(sellerCountry: string, buyerCountry: string): Promise<{ applies: boolean; reason: string }> {
    if (sellerCountry === buyerCountry) {
      return { applies: false, reason: 'Same country - reverse charge not applicable' };
    }
    const euCountries = ['DE', 'FR', 'IT', 'ES', 'NL', 'BE', 'AT', 'PT', 'IE', 'FI', 'SE', 'DK', 'PL', 'CZ', 'HU', 'RO', 'BG', 'HR', 'SK', 'SI', 'LT', 'LV', 'EE', 'CY', 'MT', 'LU', 'GR'];
  ...
  async getThresholds(): Promise<{ jurisdiction: string; threshold: number; collected: number; registered: boolean }[]> {
    const result: { jurisdiction: string; threshold: number; collected: number; registered: boolean }[] = [];
  ...
  private initializeDefaultRates(): void {
    const defaultRates: Omit<TaxRate, 'id'>[] = [
      { name: 'UK VAT Standard', type: 'vat', rate: 20, jurisdiction: 'GB', country: 'GB', active: true, inclusive: true, description: 'UK standard VAT rate' },
      { name: 'UK VAT Reduced', type: 'vat', rate: 5, jurisdiction: 'GB', country: 'GB', active: true, inclusive: true, description: 'UK reduced VAT rate' },
      { name: 'Germany VAT', type: 'vat', rate: 19, jurisdiction: 'DE', country: 'DE', active: true, inclusive: true, description: 'German standard VAT' },
      { name: 'France VAT', type: 'vat', rate: 20, jurisdiction: 'FR', country: 'FR', active: true, inclusive: true, description: 'French standard VAT' },
      { name: 'India GST', type: 'gst', rate: 18, jurisdiction: 'IN', country: 'IN', active: true, inclusive: false, description: 'Indian GST standard rate' },
      { name: 'Australia GST', type: 'gst', rate: 10, jurisdiction: 'AU', country: 'AU', active: true, inclusive: true, description: 'Australian GST' },
      { name: 'Canada GST', type: 'gst', rate: 5, jurisdiction: 'CA', country: 'CA', active: true, inclusive: false, description: 'Canadian federal GST' },
      { name: 'California Sales Tax', type: 'sales_tax', rate: 7.25, jurisdiction: 'US_CA', country: 'US', state: 'CA', active: true, inclusive: false, description: 'California state sales tax' },
      { name: 'New York Sales Tax', type: 'sales_tax', rate: 8, jurisdiction: 'US_NY', country: 'US', state: 'NY', active: true, inclusive: false, description: 'New York state + city tax' },
      { name: 'Texas Sales Tax', type: 'sales_tax', rate: 6.25, jurisdiction: 'US_TX', country: 'US', state: 'TX', active: true, inclusive: false, description: 'Texas state sales tax' },
      { name: 'Japan Consumption Tax', type: 'vat', rate: 10, jurisdiction: 'JP', country: 'JP', active: true, inclusive: true, description: 'Japanese consumption tax' },
      { name: 'Brazil ICMS', type: 'vat', rate: 18, jurisdiction: 'BR', country: 'BR', active: true, inclusive: true, description: 'Brazilian ICMS standard' },
    ];
  ...
  private getApplicableRates(country: string, state?: string, isDigitalService?: boolean): TaxRate[] {
    const rates: TaxRate[] = [];
    for (const [, rate] of this.rates) {
      if (rate.country === country && rate.active) {
        if (state && rate.state && rate.state !== state) continue;
  ...
  private checkExemption(customerId: string, jurisdiction: string): boolean {
    for (const [, exemption] of this.exemptions) {
      if (exemption.customerId === customerId && exemption.jurisdiction === jurisdiction && exemption.validUntil > Date.now()) {
        return true;
      }
    }
    return false;
